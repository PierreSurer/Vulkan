...
Scene::Scene() : physicalDevice(VK_NULL_HANDLE), currentFrame(0), framebufferResized(false) {
    glfwInit();
...
bool Scene::isDeviceSuitable(VkPhysicalDevice device) {
...
Scene::QueueFamilyIndices Scene::findQueueFamilies(VkPhysicalDevice device) {
...
int Scene::rateDeviceSuitability(VkPhysicalDevice device) {
...
Scene::SwapChainSupportDetails Scene::querySwapChainSupport(VkPhysicalDevice device) {
    Scene::SwapChainSupportDetails details;
...
VkSurfaceFormatKHR Scene::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
    for (const auto& availableFormat : availableFormats) { //TODO sort available formats
...
VkPresentModeKHR Scene::chooseSwapPresentMode(const std::vector<VkPresentModeKHR> &availablePresentModes) {
...
VkExtent2D Scene::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
...
void Scene::createSwapChain() {
    Scene::SwapChainSupportDetails swapChainSupport = querySwapChainSupport(physicalDevice);
...
void Scene::createCommandBuffers() {
    ...
    for (size_t i = 0; i < commandBuffers.size(); i += 1) {
        ...
        if (vkEndCommandBuffer(commandBuffers[i]) != VK_SUCCESS) {
            throw std::runtime_error("Command buffer save failed!");
        }
    }
...
void Scene::framebufferResizeCallback(GLFWwindow* window, int width, int height) {
    auto app = reinterpret_cast<Scene*>(glfwGetWindowUserPointer(window));
