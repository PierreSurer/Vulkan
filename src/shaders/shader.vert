...
void main() {
    float mul = 1.0f + sqrt(abs(c.offset.y));
    vec3 pos = vec3(inPosition.x * mul, inPosition.y, inPosition.z * mul) + vec3(c.offset.x, 0.0f, c.offset.z);
    ...
    gl_Position = c.transform * vec4(inPosition.x * mul, pos.y, inPosition.z * mul, 1.0);
    fragColor = inColor;
}